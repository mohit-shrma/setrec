...
bool ModelBPR::isTerminatePrecisionModel(Model& bestModel, const Data& data,
    int iter, int& bestIter, float& bestValRecall, float& prevValRecall) {

  bool ret = false;
  float currValRecall = precisionN(data.partTestMat, data.partValMat,
      data.partTrainMat, 10);
...
bool ModelBPR::isTerminatePrecisionModel(Model& bestModel, const Data& data,
  ...
  bool ret = false;
  float currValRecall = corrOrderedItems(testRatings);
...
void ModelBPR::train(const Data& data, const Params& params, Model& bestModel) {
  ...
  for (iter = 0; iter < params.maxIter; iter++) {
    ...
    for (auto&& uiRating: uiRatings) {
      ...
      negItem = sampleNegItem(data.partTrainMat, u, r_ui, mt);
