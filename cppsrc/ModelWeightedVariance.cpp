  ...
  for (int iter = 0; iter < params.maxIter; iter++) {
    ...
    for (int i = 0; i < data.nTrainSets/nTrUsers; i++) {
      for (auto&& uInd: uInds) {
        ...
        mean = ratSum/setSz;
        var = ratSqrSum/setSz - mean*mean;
        r_us_est = mean + uDivWt(user)*var;

        //user gradient
        grad = (sumItemFactors/setSz)  
          + ((uDivWt(user)/setSz) * 2.0 * ratWtSumItemFactors) 
          - ((uDivWt(user)/(setSz*setSz)) * 2.0 * ratSum * sumItemFactors); 
        grad *= 2.0*(r_us_est - r_us); 
        grad += 2.0*uReg*U.row(user).transpose();
        ...
        for (auto&& item: items) {
          temp = 1;
          temp += uDivWt(user) * 2 * tempFac.dot(V.row(item));
          temp -= (uDivWt(user)/setSz) * 2 * ratSum;
          tempGrad = temp*grad + 2.0*iReg*V.row(item).transpose(); 
          V.row(item) -= learnRate*(tempGrad.transpose());
        }
        
        //update user weight
        uDivWt(user) -= learnRate*(2.0*(r_us_est - r_us)*var 
            + 2.0*uSetBiasReg*uDivWt(user));
      }
    }    
    //objective check
