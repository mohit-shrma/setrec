...
void ModelLogisticWUm_gradCheck(void *self, int u, int *set, int setSz, float r_us) {
  ...
  commGradCoeff = -1.0*r_us/(1.0 + exp(r_us*r_us_est));
  ...
  r_us_est = avgRat - model->u_m[u];
  lossRight = log(1.0 + exp(-1.0*r_us*r_us_est));
  ...
  avgRat = dotProd(fac, sumItemLatFac, model->_(facDim))/setSz;
  r_us_est = avgRat - model->u_m[u];
  lossLeft = log(1.0 + exp(-1.0*r_us*r_us_est));
...
float ModelLogisticWUm_objective(void *self, Data *data, float **sim) {
  ...
  for (u = 0; u < data->nUsers; u++) {
    ...
    for (s = 0; s < userSet->numSets; s++) {
      ...
      diff = log(1.0 + exp(-1.0*userSetPref*userSet->labels[s]));
      se += diff;
      nSets++;
    }
...
void ModelLogisticWUm_trainRMSProp(void *self, Data *data, Params *params, 
...
void modelLogisticWUm(Data *data, Params *params, ValTestRMSE *valTest) {
  ...
  UserSets *userSet = NULL;
  ModelLogisticWUm *model = NEW(ModelLogisticWUm, "hinge loss set prediction");
